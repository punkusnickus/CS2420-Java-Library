...
public class LibraryGeneric<Type> {
	...
	public boolean checkout(long isbn, Type holder, int month, int day, int year) {
		...
		for (int i = 0; i < this.library.size(); i++) {
			if (this.library.get(i).getIsbn() == isbn) {
				LibraryBookGeneric<Type> book = this.library.get(i);
	...
	public boolean checkin(Type holder) {
		...
		for (int i = 0; i < this.library.size(); i++) {
			if (this.library.get(i).isCheckedOut())
				return false;
			if (this.library.get(i).getHolder() != null
					&& this.library.get(i).getHolder() == holder) {
				// LibraryBookGeneric<Type> book = this.library.get(i);
	...
	public ArrayList<LibraryBookGeneric<Type>> getOverdueList(int month,
		...
		ArrayList<LibraryBookGeneric<Type>> libraryCopy = new ArrayList<LibraryBookGeneric<Type>>();
		GregorianCalendar today = new GregorianCalendar(month, day, year);
		
		for(LibraryBookGeneric<Type> b : this.library)
		{
			if (b.getDueDate() != null && b.getDueDate().before(today))
				libraryCopy.add(b);
		}
	...
	protected class OrderByAuthor implements
		...
		public int compare(LibraryBookGeneric<Type> lhs,
				LibraryBookGeneric<Type> rhs) {
			int com = (int) (lhs.getAuthor().compareTo(rhs.getAuthor()));
			if (com == 0)
				com = (int) (lhs.getTitle().compareTo(rhs.getTitle()));
			return com;
